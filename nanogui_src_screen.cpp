...
static float get_pixel_ratio(GLFWwindow *window) {
    ...
    if (currentDesktop && currentDesktop == std::string("KDE")) {
        ...
        if (fscanf(fp, "%f", &ratio) != 1)
            return 1;
    } else if(currentDesktop && currentDesktop == std::string("i3")) {
        Display* display = XOpenDisplay(NULL);

        int screen_idx = DefaultScreen(display);
        int screen_width_mm = DisplayWidthMM(display,screen_idx);
        int screen_width = DisplayWidth(display, screen_idx);

        int default_dpi = 96;
        float dpi = float(screen_width) / ( float(screen_width_mm) / 25.4f); // mm => inch
        int current_dpi = (int)(dpi + 0.5);
        ratio = float(current_dpi) / float(default_dpi);
        return ratio;
    } else {
...
void Screen::drawWidgets() {
    ...
    if (elapsed > 0.2f) {
        ...
        if (widget && !widget->tooltip().empty()) {
            ...
            nvgGlobalAlpha(mNVGContext, 0.8f);
...
bool Screen::cursorPosCallbackEvent(double x, double y) {
    ...
    try {
        if (!mDragActive) {
            ...
            mCursor = widget ? widget->cursor() : Cursor::Arrow;
